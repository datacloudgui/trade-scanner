...
- **T5.3 (FABLE xhigh)** — ✅ 2026-06-11. factor_files neutros escritos para AAPL/IBM (`19840907,1,1,1` y `19620102,1,1,1`); map_files reutilizados del repo LEAN; zips Stooq AAPL/IBM reemplazaron los zips LEAN (que terminaban 2021-03-31) en `data/equity/usa/daily/`. Cross-check **redefinido**: Stooq provee precios **total-return adjusted** (dividendos + splits) → comparar vs `spy.zip` LEAN (raw) no es válido (diferencia 7-23% sistémica). Cross-check implementado como: fidelidad CSV→zip (20 fechas, ±1 unidad ×10000) + SMAs coherentes vía SymbolData (D/W listos; M:200 no-ready esperado desde 2005). Hallazgo crítico: **T6.3 debe validar en TradingView con vista "Adjusted" (total-return)**. SPY `data/equity/usa/daily/spy.zip` conserva LEAN (T4 verde). Detalle en [etapa-05b-t5-3-decisiones-y-pendientes.md](etapa-05b-t5-3-decisiones-y-pendientes.md). run_tests.sh → 35 passed.
