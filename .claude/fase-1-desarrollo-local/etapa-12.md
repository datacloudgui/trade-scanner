...
### T8b — Alinear CLAUDE.md, `config.yaml`, roadmap §0.2 y README con los hallazgos de T8

**Contexto (2026-09-23):** T8 se cerró con CLAUDE.md tal como se probó; sus hallazgos (bitácora, sección T8) se alinean aquí, antes de T9, para que el primer change real (Etapa 13) no los repita. Tarea agregada con aprobación del usuario (paso 6 del flujo). Toca el paso 5.1 del *Flujo de trabajo*: edición explícita de CLAUDE.md aprobada por el usuario. El hallazgo 4 (socket de Docker) quedó cerrado en la máquina y no entra.
...
- roadmap §0.2, fila G6 (L57): igual que L88.

**2. Otras alineaciones (hallazgos 2, 3 y 5–9):**
1. **G3 selecciona dev explícitamente:** `lean backtest "trade-scanner" --parameter env dev` en CLAUDE.md (tabla de gates, L64) y en roadmap §0.2 (L53). README L93 y L204 dejan de decir que `config.json` trae `dev`: el versionado es `prod` y dev se pide por parámetro.
2. **G3 contamina `storage/results/`:** en CLAUDE.md, *Ramas y commits*, regla 3 (L125): tras un G3 no se ejecuta `notify_email.py` sin re-correr prod (la corrida semanal desde `main` lo regenera).
3. **Gates en changes sin código:** la tabla de gates de CLAUDE.md aclara que G2 y G3 aplican aunque el change no toque código (ningún `/opsx:*` ejecuta tests ni backtests).
4. **Regla de Scenarios** (`config.yaml` L29 y CLAUDE.md L79): "… o evidencia de comando citada en `bitacora.md` (requisitos documentales o de proceso)"; el WARNING de `verify` se justifica en `design.md`.
5. **Opciones de `/opsx:archive`:** CLAUDE.md, *Reglas de archive*: si ya está sincronizado, se elige *Archive now* (*Sync anyway* solo si la spec principal difiere del delta).
6. **Rama (solo aclaración):** CLAUDE.md L75 y `config.yaml` L18/L23: "(o la rama de la etapa si el change es una tarea de una etapa sin change)".
7. **`lean.json`:** gotcha en CLAUDE.md, *Descubiertos en Etapa 12*: el CLI reescribe `file-database-last-update` en cada backtest → `git restore lean.json` antes de commitear.

**Criterio de aceptación:**
- `grep -c "se escribe su \`Purpose\`\|con el Purpose escrito\|se archiva con \`Purpose: TBD\`" CLAUDE.md` = 0 y `grep -c "escribir su Purpose" openspec/config.yaml` = 0.
- `grep -c "## Purpose" openspec/config.yaml` ≥ 1 y la misma regla en CLAUDE.md, *Artefactos de un change*.
- `grep -c -- "--parameter env dev" CLAUDE.md` ≥ 1 y `grep -c -- "--parameter env dev" .claude/fase-1-desarrollo-local/roadmap-definitivo-2026-09.md` ≥ 1; `grep -c '{"env": "dev"}' README.md` = 0.
- CLAUDE.md contiene: la regla de no publicar tras G3, G2/G3 en changes sin código, la regla de Scenarios ampliada, *Archive now*, la aclaración de rama y el gotcha de `lean.json`.
- En un change temporal (`openspec new change tmp-t8b-check`, se borra después, sin commit): `openspec instructions specs` y `openspec instructions proposal --change tmp-t8b-check` no avisan "ignoring" y muestran las reglas nuevas.
- `openspec validate --all --strict` exit 0; `bash scripts/run_tests.sh` exit 0.

**Cierre:** un commit `[Etapa 12] T8b: …` + push; T8b `[x]` con evidencia en el Done when de esta spec, en PLAN.md y en una sección T8b de la bitácora.

**Notas:** los archivos de `.claude/commands/opsx/` no se editan (los genera `openspec update`). `etapa-12.md` T8 no se reescribe (es historia de la tarea).
...
- [x] T8b: CLAUDE.md, `config.yaml`, roadmap §0.2 y README alineados con los hallazgos de T8 (Purpose en el delta y verificado tras archive; G3 con `--parameter env dev`; gotcha de `lean.json`; no publicar tras G3; G2/G3 en changes sin código; regla de Scenarios; *Archive now*; aclaración de rama); `openspec instructions` sin "ignoring"; `validate --all --strict` y `run_tests.sh` exit 0 *(2026-09-23: greps de Purpose viejo = 0 en CLAUDE.md y `config.yaml`; `## Purpose` = 1 en `config.yaml` + CLAUDE.md L83; `--parameter env dev` = 1 en CLAUDE.md y en el roadmap; `{"env": "dev"}` = 0 en README; las 6 reglas presentes en CLAUDE.md (L70, L77, L82, L92, L133, L249); `openspec instructions specs|proposal --change tmp-t8b-check` exit 0, 0 "ignoring", reglas nuevas visibles; change temporal borrado sin commit; `validate --all --strict` exit 0; `run_tests.sh` exit 0, 304 passed)*
