...
- [ ] T8b: CLAUDE.md, `config.yaml`, roadmap §0.2 y README alineados con los hallazgos de T8 (Purpose en el delta y verificado tras archive; G3 con `--parameter env dev`; gotcha de `lean.json`; no publicar tras G3; G2/G3 en changes sin código; regla de Scenarios; *Archive now*; aclaración de rama); `openspec instructions` sin "ignoring"; `validate --all --strict` y `run_tests.sh` exit 0
