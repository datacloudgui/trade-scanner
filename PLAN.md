...
- [x] T6/T7 — *(2026-09-23: T6 `53e151b`; T7 CLAUDE.md sin `PLAN.md > SPECS.md`, sección OpenSpec completa, `run_tests.sh` 304 passed)* · `AGENTS.md` de solo lectura retirado; `run_review.sh` opcional (Codex no disponible) y apuntando a la ruta nueva; CLAUDE.md con reglas OpenSpec + modelo de ramas + disposición de la documentación + revisión por defecto `/opsx:verify` + `/code-review`
- [x] T8 — spike `chore-openspec-smoke`: ciclo completo con `/opsx:*` (validate antes de archive, Purpose, retiro manual) sin flags prohibidos ni requisitos duplicados *(2026-09-23: `validate --strict` exit 0 en G0 y antes de archive; `/opsx:verify` 0 CRITICAL; requisito = 1; archivado `openspec/changes/archive/2026-09-23-chore-openspec-smoke/` (`9a9784a`); Purpose copiado del delta, sin TBD; `validate --all --strict` exit 0 antes y después del retiro manual)*
- [x] T8b — alinear CLAUDE.md, `config.yaml`, roadmap §0.2 y README con los hallazgos de T8 (Purpose en el delta, G3 `--parameter env dev`, gotcha `lean.json`, no publicar tras G3, G2/G3 sin código, regla de Scenarios, *Archive now*, rama); `openspec instructions` sin "ignoring" *(2026-09-23: greps de aceptación OK; `openspec instructions specs|proposal --change tmp-t8b-check` sin "ignoring" y con las reglas nuevas; `validate --all --strict` exit 0; `run_tests.sh` 304 passed)*
